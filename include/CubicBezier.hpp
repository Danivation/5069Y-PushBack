...
public:
    ...
    float estXDerivative(float t);
    float estYDerivative(float t);
    float estXSecondDerivative(float t);
    float estYSecondDerivative(float t);

    float getLinearVelocity(float t);
    float getAngularVelocity(float t, float linear);

    float findClosestT(VecPose currentPose);
...
public:
    ...
    float y;
    float velocity;
    VelocityPoint(float x, float y, float velocity) {
        this->x = x;
        this->y = y;
        this->velocity = velocity;
    }
    float distance(VelocityPoint p) {
        return sqrt((this->x - p.x) * (this->x - p.x) + (this->y - p.y) * (this->y - p.y));
    }
};

#define VelocityPointList std::vector<VelocityPoint>
#define BezierChain std::vector<CubicBezier>
#define VelocityList std::vector<VelocityPair>
#define VelocityChain std::vector<VelocityList>
...
struct VelocityPair
{
  ...
  float angular;
  /**
  VelocityPair(float linear, float angular)
  {
    this->linear = linear;
    this->angular = angular;
  }
  VelocityPair calculateVelocities(CubicBezier path, float t)
  {
    float tLinear = path.getLinearVelocity(t);
    float tAngular = path.getAngularVelocity(t);
    return VelocityPair(tLinear, tAngular);
  }
  /**/
};

class VelocityChainC {
public:
    std::vector<std::vector<VelocityPair>> chain;
    VelocityChainC(std::vector<std::vector<VelocityPair>> chain) {
        this->chain = chain;
    }
};